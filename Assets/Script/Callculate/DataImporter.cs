...
public class DataImporter : EditorWindow
{
    ...
    private void ImportIngredientsFromCSV(TextAsset csvFile)
    {
        ...
        foreach (string line in lines.Skip(1)) // 跳过标题行
        {
            ...
            try
            {
                ...
                config.IngredientID = int.Parse(fields[0]);
                config.IngredientName = fields[1].Trim();
                ...
                // 保存资产
                string safeName = config.IngredientName.Replace(" ", "_");
    ...
    private void BuildIngredientCache()
    {
        ...
        foreach (string guid in guids)
        {
            ...
            if (config != null)
            {
                string key = config.IngredientName.Trim();
